...
- When adding faculty, ask for: name, max credits, min credits, unique course limit, max days (default 5), and availability times in the format "MON:08:00-17:00,WED:08:00-17:00".
...
class _AddCourseSchema(BaseModel):
    ...
    rooms: str = Field(description="Comma-separated list of acceptable room names (e.g. 'Roddy 136,Roddy 205')")
    labs: str = Field(description="Comma-separated list of acceptable lab names, or empty string if none")
    faculty: str = Field(description="Comma-separated list of faculty names, or empty string if none")
...
class _ModifyCourseSchema(BaseModel):
    ...
    field: str = Field(description="Field to modify: 'credits', 'room', 'lab', or 'faculty'")
    value: str = Field(description="New value. For credits: a number. For room/lab/faculty: comma-separated list.")

class _AddFacultySchema(BaseModel):
    name: str = Field(description="Faculty member's name")
    max_credits: int = Field(description="Maximum credit hours they can teach")
    min_credits: int = Field(description="Minimum credit hours they must teach")
    unique_course_limit: int = Field(description="Maximum number of different courses they can teach")
    max_days: int = Field(default=5, description="Maximum days per week they can teach (0-5)")
    times: str = Field(description="Availability times in format 'MON:08:00-17:00,WED:09:00-18:00'")
...
class _ModifyFacultySchema(BaseModel):
    name: str = Field(description="Faculty member's name")
    field: str = Field(description="Field to modify: 'maximum_credits', 'minimum_credits', 'unique_course_limit', or 'maximum_days'")
...
class ChatbotController:
    ...
    def __init__(self, lab_model, room_model, course_model, faculty_model, conflict_model):
    ...
    @requires_config
    def _add_course(self, course_id: str, credits: int, rooms: str, labs: str, faculty: str) -> str:
        try:
            room_list = [r.strip() for r in rooms.split(",") if r.strip()]
            lab_list = [l.strip() for l in labs.split(",") if l.strip()]
    ...
    def _delete_course(self, course_id: str) -> str:
        return f"Course '{course_id}' deleted." if self.course_model.delete_course(course_id) \
    ...
    def _get_course_details(self, course_id: str) -> str:
        ...
        return (f"{course.course_id}: {course.credits} credits | rooms: {rooms} | "
    ...
    def _parse_times(times_str: str) -> dict:
        ...
        for part in times_str.split(","):
            ...
            result.setdefault(day, []).append(TimeRange(start=start.strip(), end=end.strip()))
        return result

    @requires_config
    def _add_faculty(self, name: str, max_credits: int, min_credits: int,
                     unique_course_limit: int, max_days: int, times: str) -> str:
        try:
    ...
    def _modify_faculty(self, name: str, field: str, value: int) -> str:
        valid_fields = {"maximum_credits", "minimum_credits", "unique_course_limit", "maximum_days"}
        if field not in valid_fields:
            return f"Invalid field '{field}'. Valid fields: {', '.join(sorted(valid_fields))}."
    ...
    def _get_faculty_details(self, name: str) -> str:
        ...
        return (f"{f.name}: max {f.maximum_credits} cr, min {f.minimum_credits} cr, "
    ...
    def _add_conflict(self, course_id_1: str, course_id_2: str) -> str:
        ...
        return f"Failed to add conflict (courses may not exist or conflict already exists)."
    ...
    def _delete_conflict(self, course_id_1: str, course_id_2: str) -> str:
        ...
        return f"Failed to remove conflict (may not exist)."

    @requires_config
    def _modify_conflict(self, old_course_id_1: str, old_course_id_2: str,
                         new_course_id_1: str, new_course_id_2: str) -> str:
        ...
        return f"Failed to modify conflict (courses may not exist or conflict not found)."
    ...
    def _build_agent(self):
        ...
        tools = [
            # Labs
            StructuredTool.from_function(name="add_lab",          func=self._add_lab,          description="Add a new lab",                               args_schema=_AddOrRemoveSchema),
            StructuredTool.from_function(name="delete_lab",        func=self._delete_lab,        description="Delete a lab",                                args_schema=_AddOrRemoveSchema),
            StructuredTool.from_function(name="rename_lab",        func=self._rename_lab,        description="Rename a lab",                                args_schema=_RenameSchema),
            StructuredTool.from_function(name="get_labs",          func=self._get_labs,          description="List all labs",                               args_schema=_GetSchema),
            # Rooms
            StructuredTool.from_function(name="add_room",          func=self._add_room,          description="Add a new room",                              args_schema=_AddOrRemoveSchema),
            StructuredTool.from_function(name="delete_room",        func=self._delete_room,        description="Delete a room",                               args_schema=_AddOrRemoveSchema),
            StructuredTool.from_function(name="rename_room",        func=self._rename_room,        description="Rename a room",                               args_schema=_RenameSchema),
            StructuredTool.from_function(name="get_rooms",          func=self._get_rooms,          description="List all rooms",                              args_schema=_GetSchema),
            # Courses
            StructuredTool.from_function(name="add_course",         func=self._add_course,         description="Add a new course",                            args_schema=_AddCourseSchema),
            StructuredTool.from_function(name="delete_course",      func=self._delete_course,      description="Delete a course by ID",                       args_schema=_DeleteCourseSchema),
            StructuredTool.from_function(name="modify_course",      func=self._modify_course,      description="Modify a course field (credits, room, lab, faculty)", args_schema=_ModifyCourseSchema),
            StructuredTool.from_function(name="get_courses",        func=self._get_courses,        description="List all course IDs",                          args_schema=_GetSchema),
            StructuredTool.from_function(name="get_course_details", func=self._get_course_details, description="Get detailed info for a specific course by ID",  args_schema=_CourseDetailsSchema),
            # Faculty
            StructuredTool.from_function(name="add_faculty",        func=self._add_faculty,        description="Add a new faculty member",                    args_schema=_AddFacultySchema),
            StructuredTool.from_function(name="delete_faculty",     func=self._delete_faculty,     description="Delete a faculty member by name",             args_schema=_DeleteFacultySchema),
            StructuredTool.from_function(name="modify_faculty",     func=self._modify_faculty,     description="Modify a faculty field (maximum_credits, minimum_credits, unique_course_limit, maximum_days)", args_schema=_ModifyFacultySchema),
            StructuredTool.from_function(name="get_faculty",         func=self._get_faculty,         description="List all faculty names",                                                args_schema=_GetSchema),
            StructuredTool.from_function(name="get_faculty_details", func=self._get_faculty_details, description="Get detailed info for a specific faculty member by name", args_schema=_FacultyDetailsSchema),
            # Conflicts
            StructuredTool.from_function(name="add_conflict",       func=self._add_conflict,       description="Add a conflict between two courses",                        args_schema=_ConflictSchema),
            StructuredTool.from_function(name="delete_conflict",    func=self._delete_conflict,    description="Remove a conflict between two courses",                     args_schema=_ConflictSchema),
            StructuredTool.from_function(name="modify_conflict",    func=self._modify_conflict,    description="Replace one or both courses in an existing conflict pair",  args_schema=_ModifyConflictSchema),
            StructuredTool.from_function(name="get_conflicts",      func=self._get_conflicts,      description="List all course conflicts",                                 args_schema=_GetSchema),
        ]
