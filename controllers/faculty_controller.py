...
class FacultyController:
    ...
    def add_faculty(self, faculty_data: dict) -> tuple[bool, str]:
        ...
        try:
            ...
            return (
                False,
                f"Failed to add faculty — '{faculty_data['name']}' may already exist.",
            )
    ...
    def _handle_modification(self, faculty_name: str, choice: str, faculty) -> bool:
        ...
        if choice == "1":
            ...
            if is_full_time:
                self.model.modify_faculty(
                    faculty_name, "maximum_credits", FULL_TIME_MAX_CREDITS
                )
                self.model.modify_faculty(
                ...
                self.model.modify_faculty(
                    faculty_name, "maximum_credits", ADJUNCT_MAX_CREDITS
                )
                self.model.modify_faculty(
            ...
            new_max = self.view.get_integer_input(
                "Enter new maximum credits: ", min_val=faculty.minimum_credits
            )
            ...
            new_limit = self.view.get_integer_input(
                "Enter new unique course limit: ", min_val=1
            )
            return self.model.modify_faculty(
                faculty_name, "unique_course_limit", new_limit
            )
        elif choice == "5":
            new_max_days = self.view.get_integer_input(
                "Enter new maximum days (0-5): ", min_val=0, max_val=5
            )
            return self.model.modify_faculty(faculty_name, "maximum_days", new_max_days)
        elif choice == "6":
            self.view.display_message(
                "Availability time modification not yet implemented in MVC."
            )
            return False
        elif choice == "7":
            course_prefs = self.view.get_course_preferences_input()
            return self.model.modify_faculty(
                faculty_name, "course_preferences", course_prefs
            )
        elif choice == "8":
            room_prefs = self.view.get_room_preferences_input()
            return self.model.modify_faculty(
                faculty_name, "room_preferences", room_prefs
            )
        elif choice == "9":
