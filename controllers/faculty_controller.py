...
class FacultyController:
    ...
    def cli_add_faculty(self):
        """CLI workflow for adding faculty. Prompts via self.view."""
        try:
    ...
    def cli_delete_faculty(self):
        """CLI workflow for deleting faculty. Prompts via self.view."""
        try:
    ...
    def modify_faculty(self):
        """CLI workflow for modifying faculty. Prompts via self.view."""
        try:
            faculty_list = self.model.get_all_faculty()
            if not faculty_list:
            ...
            faculty      = self.model.get_faculty_by_name(faculty_name)
            if not faculty:
                self.view.display_error(f"No faculty '{faculty_name}' found.")
                return
            ...
            if success:
                self.view.display_message(f"Faculty '{faculty_name}' modified successfully.")
            else:
