...
class ScheduleController:
    ...
    def import_schedule_file(self, fileName, fileData):
        ...
        if schedules and self.model.config_model:
            self.model.config_model.config_path = os.path.join(
                os.path.dirname(self.model.config_model.config_path), fileName
            )
    ...
    def run_scheduler(self):
        ...
        try:
            ...
            save_to_file = self.view.confirm(
                "Do you want to save the schedule to a file?"
            )
            ...
            if os.path.exists(output_file):
                if self.view.confirm(
                    ...
                    if not self.view.confirm(
                        "Would you like to choose a different filename?"
                    ):
                        return (None, False)
                    continue
    ...
    def _generate_and_output(
        self, limit: int, output_file: str | None, format_is_csv: bool
    ):
        """
        ...
        try:
            with open(output_file, "w") as f:
                if format_is_csv:
                    ...
                    json.dump({"generatedSchedules": schedule_list}, f, indent=4)

            self.view.display_message(
                f"Schedules successfully written to {output_file}"
            )
