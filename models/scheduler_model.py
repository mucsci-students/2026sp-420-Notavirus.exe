...
class SchedulerModel:
    ...
    def _build_dummy_course(
        self, course_str: str, faculty: str | None = None, lab: str | None = None
    ):
        """
    ...
    def import_from_csv(self, file_bytes: bytes):
        ...
        for row in reader:
            ...
            dummy_course = self._build_dummy_course(course_str, faculty=faculty)

            ci = CourseInstance(
                course=dummy_course,
                time=TimeSlot(times=times, lab_index=lab_index),
                faculty=faculty,
                room=room,
                lab=lab,
            )
