...
def modifyConflict_getCourses(modifyNum: int, config: CombinedConfig):
    ...
    for courseIdx, course in enumerate(config.config.courses):
        ...
        if conflictCounter == modifyNum:
            break
    return (
        config.config.courses[courseIndex],
        config.config.courses[courseIndex].conflicts[conflictIndex],
    )
...
def modifyConflict_JSON(
    selectedCourse: CourseConfig,
    targetConflictCourse: CourseConfig,
    targetNewCourse: CourseConfig,
    modifyMode: int,
    ...
    if modifyMode == 1:
        # A-B -> C-B
        # Remove B from A
        selectedCourse.conflicts = [
            c for c in selectedCourse.conflicts if c != selectedConflict_id
        ]
        ...
        updated = [
            newCourse_id if c == old_course_id else c
            for c in targetConflictCourse.conflicts
        ]
        ...
        updated = [
            newCourse_id if c == selectedConflict_id else c
            for c in selectedCourse.conflicts
        ]
        selectedCourse.conflicts = list(dict.fromkeys(updated))

        # Remove A from B (targetConflictCourse)
        targetConflictCourse.conflicts = [
            c for c in targetConflictCourse.conflicts if c != old_course_id
        ]
...
def select_course_instance(
    course_id: str, config: CombinedConfig, prompt_msg: str
) -> CourseConfig:
...
def modifyconflict_input(config: CombinedConfig, config_path: str):
    ...
    while True:
        modifyNum_str = input(
            "Which conflict would you like to modify? [1 - " + str(conflictNum) + "]: "
        ).strip()
    ...
    targetConflictCourse = select_course_instance(
        selectedConflict_str,
        config,
        f"Select the specific instance of {selectedConflict_str} involved in this conflict",
    )
    ...
    if modifyNum == "1":
        ...
        newCourse_name = (
            input(f"Replace course {selectedCourse.course_id} with (Enter Course ID): ")
            .strip()
            .upper()
        )
    else:
        ...
        newCourse_name = (
            input(
                f"Replace course {targetConflictCourse.course_id} with (Enter Course ID): "
            )
            .strip()
            .upper()
        )
    ...
    targetNewCourse = select_course_instance(
        newCourse_name, config, f"Select the specific instance of {newCourse_name}"
    )
    ...
    while True:
        confirm = input(
            "Are you sure you want to modify this conflict? [y/n]: "
        ).strip()
...
def deleteConflict(config: CombinedConfig, config_path: str):
    ...
    for i, course in enumerate(scheduler_config.courses):
        for conflict_id in course.conflicts:
            ...
            conflict_label = next(
                (
                    course_section_labels[j]
                    for j, c in enumerate(scheduler_config.courses)
    ...
    while True:
        ...
        if course_1 == "":
            ...
            print(
                f"Error: '{course_1}' is not a valid course. Valid courses are: {', '.join(sorted(valid_courses))}"
            )
        else:
            break
    ...
    while True:
        course_2 = (
            input("Enter the conflicting course ID (e.g. 'CMSC 340'): ").strip().upper()
        )
        if course_2 == "":
            print("Course ID cannot be empty.")
        elif course_2 not in valid_courses:
            print(
                f"Error: '{course_2}' is not a valid course. Valid courses are: {', '.join(sorted(valid_courses))}"
            )
        elif course_2 == course_1:
            print("Error: A course cannot conflict with itself.")
        elif tuple(sorted([course_1, course_2])) not in [
            (a.rsplit(".", 1)[0], b.rsplit(".", 1)[0]) for a, b in existing_conflicts
        ]:
