...
def add_lab(existing_labs=None):
    ...
    if confirm == "y":
        ...
        while True:
            restart = (
                input("Would you like to restart adding the lab? [y/n]: ")
                .lower()
                .strip()
            )
            if restart in ("y", "n"):
...
def modifyLab(labs, courses, faculty):
    ...
    while True:
        old_name = input(
            "Enter the name of the lab you would like to modify, must be entered how it is shown above. (or press Enter to cancel): "
        ).strip()
        if old_name == "":
            print("Cancelled")
            return labs, courses, faculty
    ...
    if affected_courses:
        print(
            f"  -{len(affected_courses)} course(s): {','.join(c.course_id for c in affected_courses)}"
        )
    if affected_faculty:
        print(
            f"  -{len(affected_faculty)} faculty: {','.join(f.name for f in affected_faculty)}"
        )
    ...
    if confirm.lower() == "y":
        ...
        for course in courses:
            if old_name in course.lab:
                course.lab = [
                    new_name if lab == old_name else lab for lab in course.lab
                ]
...
def deleteLab_input(config: CombinedConfig, config_path: str):
    ...
    while True:
        confirm = input(
            ...
            + config.config.labs[int(labnum) - 1]
            + "? [y/n]: "
        ).strip()
        if confirm == "y":
            if deleteLab_json(
