...
def display_Configuration(config: scheduler.CombinedConfig):
    ...
    if config.config.courses:
        print(
            f"  {'Course ID':<15} {'Credits':<10} {'Rooms':<30} {'Labs':<20} {'Conflicts'}"
        )
        print("  " + "-" * 100)
        for course in config.config.courses:
            ...
            if hasattr(course, "room") and course.room:
                rooms = (
                    ", ".join(course.room)
                    if isinstance(course.room, list)
                    else str(course.room)
                )
            ...
            if hasattr(course, "conflicts") and course.conflicts:
                conflicts = (
                    ", ".join(course.conflicts)
                    if isinstance(course.conflicts, list)
                    else str(course.conflicts)
                )
            ...
            conflicts_display = (
                conflicts[:30] + ".." if len(conflicts) > 32 else conflicts
            )

            print(
                f"  {course_id:<15} {credits:<10} {rooms_display:<30} {labs_display:<20} {conflicts_display}"
            )
    else:
    ...
    if config.config.faculty:
        print(
            f"  {'Name':<15} {'Max Credits':<12} {'Min Credits':<12} {'Unique Limit':<13} {'Max Days':<10} {'Mandatory Days'}"
        )
        print("  " + "-" * 100)
        for faculty in config.config.faculty:
            name = faculty.name if hasattr(faculty, "name") else "N/A"
            max_cred = (
                faculty.maximum_credits
                if hasattr(faculty, "maximum_credits")
                else "N/A"
            )
            min_cred = (
                faculty.minimum_credits
                if hasattr(faculty, "minimum_credits")
                else "N/A"
            )
            unique = (
                faculty.unique_course_limit
                if hasattr(faculty, "unique_course_limit")
                else "N/A"
            )
            max_days = (
                faculty.maximum_days if hasattr(faculty, "maximum_days") else "N/A"
            )
            ...
            print(
                f"  {name:<15} {str(max_cred):<12} {str(min_cred):<12} {str(unique):<13} {str(max_days):<10} {mandatory}"
            )
        ...
        for faculty in config.config.faculty:
            ...
            if hasattr(faculty, "course_preferences") and faculty.course_preferences:
                prefs = ", ".join(
                    [f"{k}({v})" for k, v in faculty.course_preferences.items()]
                )
            ...
            if hasattr(faculty, "room_preferences") and faculty.room_preferences:
                prefs = ", ".join(
                    [f"{k}({v})" for k, v in faculty.room_preferences.items()]
                )
                print(f"      Rooms: {prefs}")
            ...
            if hasattr(faculty, "lab_preferences") and faculty.lab_preferences:
                prefs = ", ".join(
                    [f"{k}({v})" for k, v in faculty.lab_preferences.items()]
                )
                print(f"      Labs: {prefs}")
    else:
    ...
    print("\nTIME SLOT PATTERNS:")
    if hasattr(config, "time_slot_config") and hasattr(
        config.time_slot_config, "classes"
    ):
        for i, pattern in enumerate(config.time_slot_config.classes, 1):
            credits = pattern.credits if hasattr(pattern, "credits") else "N/A"
            disabled = (
                " [DISABLED]"
                if hasattr(pattern, "disabled") and pattern.disabled
                else ""
            )
            ...
            if hasattr(pattern, "meetings") and pattern.meetings:
                for meeting in pattern.meetings:
                    ...
                    duration = (
                        meeting.duration if hasattr(meeting, "duration") else "N/A"
                    )
...
def main():
    ...
    try:
        ...
        config = scheduler.load_config_from_file(
            scheduler.CombinedConfig, configFileName
        )
        runScheduler(config)
...
def promptSaveToFile():
    ...
    while True:
        save_input = (
            input("Do you want to save the schedule to a file? (yes/no): ")
            .strip()
            .lower()
        )
...
def promptFormat():
    ...
    while True:
        formatInput = (
            input("What output file format do you prefer? (csv/json): ").strip().lower()
        )
...
def promptOutputFileName():
    ...
    while True:
        fileName = input(
            "What do you want to call the output file? (without extension): "
        ).strip()
        if fileName:
    ...
    if os.path.exists(outputFile):
        while True:
            overwrite = (
                input(f"File '{outputFile}' already exists. Overwrite? (yes/no): ")
                .strip()
                .lower()
            )
...
def specifyLimit():
    ...
    while True:
        try:
            limit_input = input(
                "What is the max number of schedules you want generated? "
            ).strip()
...
def promptConfigFile():
    ...
    while True:
        fileName = input(
            "Which config file do you want to use? (Must be a CombinedConfig): "
        ).strip()
