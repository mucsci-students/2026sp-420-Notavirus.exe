...
def deleteRoom(config_path: str):
    ...
    while True:
        confirm = (
            input(f"Are you sure you want to delete room '{room_name}'? [y/n]: ")
            .lower()
            .strip()
        )
        if confirm in ("y", "n"):
    ...
    if room is not None:
        ...
        while True:
            ...
            if not existCheck(newRoom):
                ...
                print(
                    f"room: {room} already exists within {loadedConfig.config.rooms}, do not add"
                )
    ...
    while True:
        if existCheck(room):
            match modifyPrompt():
                ...
                case _:
                    print(
                        "match failed, input should be building or number only, this is a fallback"
                    )
