...
def build_test_course(
    course_id="TEST 101", credits=3, rooms=None, labs=None, faculty=None, conflicts=None
) -> CourseConfig:
    """
...
def test_add_course_success(course_model):
    ...
    assert result
    assert course_model.course_exists("TEST 101")
...
def test_add_course_empty_rooms_labs(course_model):
    ...
    """
    course = build_test_course(
        course_id="EMPTY 101", rooms=[], labs=[], faculty=["Hardy"]
    )

    result = course_model.add_course(course)

    assert result
    assert course_model.course_exists("EMPTY 101")
...
def test_delete_course_success(course_model):
    ...
    assert result
    assert not course_model.course_exists("DELETE 101")
...
def test_course_exists_true(course_model):
    ...
    assert course_model.course_exists("EXISTS 101")
...
def test_course_exists_false(course_model):
    ...
    Parameters:
        course_model (CourseModel): Course model fixture
    """
    assert not course_model.course_exists("NONEXISTENT 999")
...
def test_delete_course_by_section_index(course_model):
    ...
    assert course_model.course_exists("DELSECT 101")
