...
def test_add_lab_success(lab_model):
    ...
    assert result
    assert lab_model.lab_exists("Python Lab")
...
def test_add_lab_with_spaces(lab_model):
    ...
    assert result
    assert lab_model.lab_exists("Advanced Algorithms Lab")
...
def test_add_lab_special_characters(lab_model):
    ...
    assert result
    assert lab_model.lab_exists("Lab-C++ (Intro)")
...
def test_add_lab_numeric_name(lab_model):
    ...
    assert result
    assert lab_model.lab_exists("CS101")
...
def test_add_lab_long_name(lab_model):
    ...
    assert result
    assert lab_model.lab_exists(long_name)
...
def test_delete_lab_success(lab_model):
    ...
    assert result
    assert not lab_model.lab_exists("Delete Me Lab")
...
def test_delete_lab_removes_from_courses(lab_model):
    ...
    updated_course = [
        c for c in config_model.config.config.courses if c.course_id == "LABTEST 101"
    ][0]
...
def test_delete_lab_removes_from_faculty_preferences(lab_model):
    ...
    updated_faculty = [
        f for f in config_model.config.config.faculty if f.name == "Test Lab Faculty"
    ][0]
...
def test_modify_lab_success(lab_model):
    ...
    assert result
    assert not lab_model.lab_exists("OldLab 101")
    assert lab_model.lab_exists("NewLab 101")
...
def test_modify_lab_updates_courses(lab_model):
    ...
    updated_course = [
        c for c in config_model.config.config.courses if c.course_id == "LABUPDATE 101"
    ][0]
...
def test_modify_lab_updates_faculty_preferences(lab_model):
    ...
    updated_faculty = [
        f for f in config_model.config.config.faculty if f.name == "Update Lab Faculty"
    ][0]
...
def test_modify_lab_multiple_courses(lab_model):
    ...
    updated_course1 = [
        c for c in config_model.config.config.courses if c.course_id == "MULTI1"
    ][0]
    updated_course2 = [
        c for c in config_model.config.config.courses if c.course_id == "MULTI2"
    ][0]
...
def test_lab_exists_true(lab_model):
    ...
    assert lab_model.lab_exists("Exists Lab")
...
def test_lab_exists_false(lab_model):
    ...
    Parameters:
        lab_model (LabModel): Lab model fixture
    """
    assert not lab_model.lab_exists("NonExistent Lab")
