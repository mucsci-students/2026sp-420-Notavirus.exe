...
def test_add_room_success(room_model):
    ...
    assert result
    assert room_model.room_exists("Roddy 141")
...
def test_delete_room_success(room_model):
    ...
    assert result
    assert not room_model.room_exists("Delete Me")
...
def test_delete_room_removes_from_courses(room_model):
    ...
    updated_course = [
        c for c in config_model.config.config.courses if c.course_id == "TEST 101"
    ][0]
...
def test_delete_room_removes_from_faculty_preferences(room_model):
    ...
    updated_faculty = [
        f for f in config_model.config.config.faculty if f.name == "Test Faculty"
    ][0]
...
def test_modify_room_success(room_model):
    ...
    assert result
    assert not room_model.room_exists("OldRoom 101")
    assert room_model.room_exists("NewRoom 101")
...
def test_modify_room_updates_courses(room_model):
    ...
    updated_course = [
        c for c in config_model.config.config.courses if c.course_id == "UPDATE 101"
    ][0]
...
def test_modify_room_updates_faculty_preferences(room_model):
    ...
    updated_faculty = [
        f for f in config_model.config.config.faculty if f.name == "Update Faculty"
    ][0]
...
def test_modify_room_no_conflicts(room_model):
    ...
    assert result
    assert room_model.room_exists("Roddy 151")
...
def test_room_exists_true(room_model):
    ...
    assert room_model.room_exists("Exists Room")
...
def test_room_exists_false(room_model):
    ...
    Parameters:
        room_model (RoomModel): Room model fixture
    """
    assert not room_model.room_exists("NonExistent Room")
