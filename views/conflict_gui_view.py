...
class ConflictGUIView:
    ...
    def conflict_add():
        ...
        with ui.column().classes("w-full items-center pt-12 pb-12 gap-4"):
            ...
            course_a = (
                ui.select(labels, label="Course A", value=labels[0])
            ...
            def section_conflict_exists(label_a, label_b):
                ...
                return (
                    obj_b.course_id in obj_a.conflicts
                    or obj_a.course_id in obj_b.conflicts
                )
    ...
    def conflict_modify():
        ...
        def build_section_options():
            return controller.gui_get_conflict_labels(
                existing_conflicts, section_label_map
            )
        ...
        with ui.column().classes("w-full items-center pt-12 pb-12 font-sans gap-6"):
            ...
            ui.label(
                "Select an existing conflict and choose the new classes you want to conflict."
            ).classes("text-lg !text-black dark:!text-white text-center max-w-xl")

            if not existing_conflicts:
                ui.label(
                    "There are no conflicts currently in the configuration."
                ).classes("text-xl !text-black dark:!text-white")
                ui.button("Back").props(
                    "rounded color=black text-color=white no-caps"
                ).classes("w-80 h-16 text-xl").on(
                    "click", lambda: ui.navigate.to("/conflict")
                )
                return
            ...
            def update_selection(e):
                ...
                if val:
                    ...
                    match_a = (
                        s1
                        if s1 in current_labels
                        else next(
                            (
                                label
                                for label in current_labels
                                if label == c1 or label.startswith(c1 + ".")
                            ),
                            None,
                        )
                    )
                    match_b = (
                        s2
                        if s2 in current_labels
                        else next(
                            (
                                label
                                for label in current_labels
                                if label == c2 or label.startswith(c2 + ".")
                            ),
                            None,
                        )
                    )
            ...
            with ui.row().classes(
                "gap-4 w-full max-w-xl justify-center items-center mt-2"
            ):
                new_course_a = (
                    ui.select(all_section_labels, label="New Course A")
                    .props("label-color=grey-7")
                    .classes("w-64 max-w-xs")
                )
                new_course_b = (
                    ui.select(all_section_labels, label="New Course B")
                    .props("label-color=grey-7")
                    .classes("w-64 max-w-xs")
                )
            ...
            ui.switch(
                "Sort course conflicts without section preference", on_change=on_toggle
            ...
            def on_modify():
                ...
                if not new_c1 or not new_c2:
                    ...
                    feedback.classes(replace="text-lg text-red-600")
                    return
                success, message = controller.gui_modify_conflict(
                    old_c1, old_c2, new_c1, new_c2, i1=i1, i2=i2
                )
    ...
    def conflict_delete():
        ...
        with ui.column().classes("w-full items-center pt-12 pb-12 font-sans gap-6"):
            ...
            ui.label("Delete Conflict").classes(
                "text-4xl mb-4 !text-black dark:!text-white"
            )
            ui.label(
                "Select a conflict. Toggle the switch to ignore sections. You must check the conflict first to delete it."
            ).classes("text-lg !text-black dark:!text-white text-center max-w-xl")
            ...
            select = (
                ui.select(
                    ...
                    on_change=lambda e: selected.update(
                        {"value": conflict_options.get(e.value)}
                    ),
                )
                .props("label-color=grey-7")
                .classes("w-full max-w-xl text-xl")
            ...
            def on_toggle(e):
                ...
                select.value = None
                conflict_options.clear()
                conflict_options.update(
                    build_base_options() if e.value else build_section_options()
                )
            ...
            confirm_card = ui.card().classes(
                "w-full max-w-xl items-center bg-transparent shadow-none"
            )
            ...
            with confirm_card:
                ...
                with ui.row().classes("gap-4 justify-center mt-2"):
                    ...
                    ui.button("Confirm Delete").props(
                        "rounded color=red text-color=white no-caps"
                    ).classes("w-48 h-12 text-lg").on("click", on_confirm)
                    ui.button("Cancel").props(
                        "rounded color=black text-color=white no-caps"
                    ).classes("w-48 h-12 text-lg dark:!bg-white dark:!text-black").on(
                        "click", on_cancel
                    )
    ...
    def conflict_view():
        ...
        with ui.column().classes("w-full items-center pt-12 pb-12 gap-4"):
            ...
            ui.label("View Conflicts").classes(
                "text-4xl mb-6 !text-black dark:!text-white"
            )
